...
def inicio():
    ...
    if request.method == 'POST':
        ...
        ##
        conn = pymysql.connect(host='INNOVANTORSS.mysql.pythonanywhere-services.com', user='INNOVANTORSS', passwd='tV64eb6EGFiPN6_', db='INNOVANTORSS$default')
        cursor = conn.cursor()
        cursor.execute(
...
def cuota():
    conn = pymysql.connect(host='INNOVANTORSS.mysql.pythonanywhere-services.com', user='INNOVANTORSS', passwd='tV64eb6EGFiPN6_', db='INNOVANTORSS$default')
    cursor = conn.cursor()
    cursor.execute('select * from tabla_imss')
    cursor.execute(
        ' select a.id_Concepto, a.Descripcion, a.Porcentaje, a.id_base, b.descripcion'
        ' from tabla_imss a, base_imss b '
        ' where b.id_base=a.id_base ')

    datos = cursor.fetchall()
    conn.close()
    return render_template("tabla_cuota.html", cuota=datos)
...
def agr_cuota():
    conn = pymysql.connect(host='INNOVANTORSS.mysql.pythonanywhere-services.com', user='INNOVANTORSS', passwd='tV64eb6EGFiPN6_', db='INNOVANTORSS$default')
    cursor = conn.cursor()
    cursor.execute('select id_base, descripcion, calculo from base_imss')
    datos = cursor.fetchall()
    conn.close()
    return render_template("agrega_cuota.html", obreros_cuo=datos)


@app.route('/cuota_agr', methods=['POST'])
def cuota_agr():
    if request.method == 'POST':
        aux_Descripcion = request.form['Descripcion']
        aux_Porcentaje = request.form['Porcentaje']
        porc = float(aux_Porcentaje)
        aux_base = request.form['base']
        conn = pymysql.connect(host='INNOVANTORSS.mysql.pythonanywhere-services.com', user='INNOVANTORSS', passwd='tV64eb6EGFiPN6_', db='INNOVANTORSS$default')
        cursor = conn.cursor()
        cursor.execute(
            'select count(*) from tabla_imss where Descripcion = %s', (aux_Descripcion))
        datos1 = cursor.fetchone()
        if (porc > 100):
            error = "El porcentaje no puede ser mayor a 100%."
            return render_template("error.html", des_error=error, paginaant="/agr_cuota")
        if (porc < 0):
            error = "El porcentaje no puede ser menor a 0%."
            return render_template("error.html", des_error=error, paginaant="/agr_cuota")
        if (datos1[0] != 0):
            error = "La Cuota IMSS ya está agregada."
            return render_template("error.html", des_error=error, paginaant="/agr_cuota")
        else:
            cursor.execute('insert into tabla_imss (Descripcion, Porcentaje, id_base) values (%s, %s, %s)',
                           (aux_Descripcion, aux_Porcentaje, aux_base))
            cursor.execute('select a.id_Concepto, a.Descripcion, a.Porcentaje, a.id_base, b.descripcion'
                           ' from tabla_imss a, base_imss b '
                           ' where b.id_base=a.id_base')

            conn.commit()
            cursor.execute(
                'select descripcion from base_imss order by Descripcion')
            datos = cursor.fetchall()
            conn.close()
...
@app.route('/ed_cuota/<string:id>')
def ed_cuota(id):
    conn = pymysql.connect(host='INNOVANTORSS.mysql.pythonanywhere-services.com', user='INNOVANTORSS', passwd='tV64eb6EGFiPN6_', db='INNOVANTORSS$default')
    cursor = conn.cursor()
    cursor.execute(
        'select id_Concepto, Descripcion, Porcentaje, id_base from tabla_imss where id_Concepto=%s', (id))
    datos = cursor.fetchone()
    cursor.execute(
        'select id_base, Descripcion from base_imss order by Descripcion')
    datos1 = cursor.fetchall()

    return render_template("edi_cuota.html", cuotas=datos, obreros_cuo=datos1)


@app.route('/modifica_cuota/<string:id>', methods=['POST'])
def modifica_cuota(id):
    if request.method == 'POST':
        aux_descripcion=request.form['Descripcion']
        aux_porcentaje=request.form['porcentaje']
        por = float(aux_porcentaje)
        aux_base=request.form['base']
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('select count(*) from tabla_imss where Descripcion = %s',(aux_descripcion))
        cuo = cursor.fetchone()
        cursor.execute('select count(*) from tabla_imss where porcentaje = %s',(aux_porcentaje))
        porc = cursor.fetchone()
        cursor.execute('select count(*) from tabla_imss where id_base = %s',(aux_base))
        base = cursor.fetchone()
        if(por<0):
            error = "El porcentaje no puede ser menor a 0."
            return render_template("error.html", des_error=error, paginaant="/cuota")
        if(por>100):
            error = "El porcentaje no puede ser mayor a 100."
            return render_template("error.html", des_error=error, paginaant="/cuota")
        if (cuo[0] != 0):
            if (porc[0]!=0):
                if (base[0]!=0):
                    error = "La cuota IMSS ya se encuentra agregada."
                    return render_template("error.html", des_error=error, paginaant="/cuota")
                else:
                    cursor.execute('update tabla_imss set  Descripcion=%s, Porcentaje=%s, id_base=%s where Id_Concepto=%s', (aux_descripcion, aux_porcentaje, aux_base, id))
                conn.commit()
                conn.close()
                return redirect(url_for('cuota'))
            else:
...
@app.route('/bo_cuota/<string:id>')
def bo_cuota(id):
    conn = pymysql.connect(host='INNOVANTORSS.mysql.pythonanywhere-services.com', user='INNOVANTORSS', passwd='tV64eb6EGFiPN6_', db='INNOVANTORSS$default')
    cursor = conn.cursor()
    cursor.execute('delete from tabla_imss where id_Concepto = {0}'.format(id))
    conn.commit()
    conn.close()
    return redirect(url_for('cuota'))
...
def agrega_det_nomina(id):
    if request.method == 'POST':
        ...
        if (det[0] != 0):
            ...
            cursor.execute('insert into detalle_nomina (id_calnomina, Curp, salario_base, tot_per, isr, imss, tot_ded, salario_neto, tot_grabado) values (%s,%s, %s, %s, %s, %s, %s, %s, %s)',
                           (id, aux_curp, aux_base, aux_tot_per, aux_isr, aux_imss, aux_tot_ded, aux_neto, aux_grabado))
            conn.commit()
            cursor.execute('select id_calnomina, Curp, Salario_base, tot_per, isr, imss, tot_ded, salario_neto, tot_grabado '
            ...
            cursor.execute('select a.id_calnomina, a.Curp, b.Id_Percepcion, b.Descripcion, c.id_calnomina, c.Id_Percepcion, c.Grabado, c.monto'
            ...
            datos6 = cursor.fetchall()
            conn.close()
        return render_template("edi_det_nomina.html", detalle=dato, nomina=datos, ded_nomina=datos2, per_nomina=datos1, percepciones=datos3, deducciones=datos4, curp=datos5, empleado=datos6)
...
def ed_det_nomina(id, curp):
    ...
    datos7 = cursor.fetchall()
    conn.close()
    return render_template("edi_det_nomina.html", detalle=datos, nomina=datos1, per_nomina=datos2, ded_nomina=datos3, percepciones=datos4, deducciones=datos5, curp=datos6, empleado=datos7)
...
def modifica_det_nomina(id, curp):
    if request.method == 'POST':
        ...
        cursor.execute('update detalle_nomina set Salario_base=%s, tot_per=%s, isr=%s, imss=%s, tot_ded=%s, salario_neto=%s, tot_grabado=%s '
                           ' where id_calnomina=%s and Curp=%s', (aux_base, aux_tot_per, aux_isr, aux_imss, aux_tot_ded, aux_neto, aux_grabado, id, curp))
        conn.commit()
        conn.close()
    return redirect(url_for('nomina'))
...
def cal_det_isr(id, curp, sb, per):
    ...
    print("---------------------------------------------;-;",str(sb))
    sal_bru=float(sb)
    print("-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-","Salario bruto", sal_bru)
    perce=float(per)
    print("-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-","Percepciones", perce)
    isrp=sal_bru + perce
    print("Suma-----------------------------------------------|||||||||||||||", isrp)
...
'''@app.route('/bo_det_nomina/<string:id>')
def bo_det_nomina(id):
    conn = pymysql.connect(host='INNOVANTORSS.mysql.pythonanywhere-services.com', user='INNOVANTORSS', passwd='tV64eb6EGFiPN6_', db='INNOVANTORSS$default')
    cursor = conn.cursor()
    cursor.execute('delete from per_nomina where id_percepcion = %s',(id))
    conn.commit()
    cursor.execute('delete from ded_nomina where id_deduccion = %s',(id))
    conn.commit()
    cursor.execute('delete from detalle_nomina where id_calnomina = %s',(id))
    conn.commit()
    conn.close()
    return redirect(url_for('nomina'))'''
...
@app.route('/agrega_per_nomina/<string:curp>', methods=['POST'])
def agrega_per_nomina(curp):
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='INNOVANTORSS.mysql.pythonanywhere-services.com', user='INNOVANTORSS', passwd='tV64eb6EGFiPN6_', db='INNOVANTORSS$default')
        cursor = conn.cursor()
        cursor.execute('select count(*) from per_nomina where curp = %s and id_percepcion = %s and id_calnomina = %s',
                       (aux_curp, aux_per, aux_cal_nomina))
        existe_perpues = cursor.fetchone()
        if (existe_perpues[0] != 0):
            ...
            datos6 = cursor.fetchall()
            conn.close()
            return render_template("edi_det_nomina.html", detalle=datos, per_nomina=datos1, ded_nomina=datos2, percepciones=datos3, deducciones=datos4, curp=datos6, nomina=datos7)
...
def bo_per_nomina(idN, curp, idP):
    ...
    cursor = conn.cursor()
    cursor.execute(
        'delete from per_nomina where id_calnomina = %s and curp = %s and id_percepcion = %s', (idN, curp, idP))
    conn.commit()
    cursor.execute('select id_calnomina, curp, salario_base, tot_per, isr, imss, tot_ded, salario_neto, tot_grabado '
    ...
    datos6 = cursor.fetchall()
    conn.close()
    return render_template("edi_det_nomina.html", detalle=datos, per_nomina=datos1, ded_nomina=datos2, percepciones=datos3, deducciones=datos4, curp=datos6)
